...
async def token_updater():
    while True:
        await get_jwt_token()
        await asyncio.sleep(8 * 3600)  # تحديث الـ JWT كل 8 ساعات

async def get_player_information(player_id):
    global jwt_token
    if not jwt_token:
        return f"JWT token is missing for player {player_id}"
...
async def start_requests():
    ...
    player_ids = [int(uid)] * 100  # إرسال 1000 طلب لنفس الـ UID
    ...
    return jsonify({
        ...
        "status": "success",
        "message": "100 requests sent successfully",
        "results": results
    })
